...
If you have not downloaded the data from the [Dropbox link](https://www.dropbox.com/scl/fo/sft7jj8edccwv0hhf8c0w/ADCpTvLbdWi0Hq1IHtc-Q5g?rlkey=du6hzjzqbwgurew12yzfk37uw&dl=0), be sure to do so.

We'll import three datasets in this workflow, one containing actual data from the eddy covariance tower for several years. <br>
...
# SUMMARISE data to daily intervals
usfs_met_rh <- usfs_met %>%
  group_by(Date = as.Date(TIMESTAMP)) %>%
  BLANK(Tmean = mean(AirT..C.),
            Tmax = max(AirT..C.),
            Tmin = min(AirT..C.),
            RHmean = mean(RH....),
            RHmax = max(RH....),
...
```{r}
# Calculate daily precipitation by subtracting consecutive values, use mutate() to create a new column for the calculation.
usfs_met <- usfs_met %>%
  BLANK(daily_ppt_mm = Cumulative_ppt..mm. - lag(Cumulative_ppt..mm., default = 0))
...
usfs_met <- usfs_met %>%
  BLANK(daily_ppt_mm = if_else(TIMESTAMP == ymd_hms("2022-01-01 00:00:00"), 0, daily_ppt_mm))
...
# Convert cfs to cubic millimeters after grouping by day
lfc_weir <- lfc_weir %>%
  BLANK(Date = as.Date(TIMESTAMP)) %>%
  summarise(
...
Note that ET data is in mmol/m2/s (mmol H2O per square meter per second). We want to convert this to mm/day. <br>
...
# Convert 'datetime' column to POSIXct type using lubridates mdy_mm
usfs_et$datetime <- BLANK(usfs_et$datetime)

# Extract date from datetime
usfs_et$date <- as.Date(usfs_et$datetime)

# Group by date and calculate sum of 'ET_mm' (not mutate())
usfs_et_daily <- usfs_et %>%
  group_by(date) %>%
  BLANK(ET_mm = sum(ET_mm))
...
ggplot() +
  ...
  geom_line(data = BLANK, aes(x = index, y = aet_pt, color = "PT Method")) +
...
**Q4. (3 pnts) In the context of evapotranspiration modeling, discuss how vegetation type and soil properties might interact to influence AET in the Fraser Experimental Forest. Provide a hypothetical example of how these factors could vary between two sub-watersheds in the area.**

ANSWER:

**Q5 (5 pnts) Let's assume we do not have eddy covariance tower data for this time period, but you have the provided discharge and precipitation measurements. Which model would you choose to estimate an annual ET sum and why?** 

ANSWER:

**Q6 (3 pnts) In our modeling script, while our main objective was to estimate evapotranspiration (ET), which specific aspect required the most labor and attention to detail? Reflect on the tasks involved in data preparation, cleaning, and formatting. How did these preliminary steps impact the overall modeling process? **
